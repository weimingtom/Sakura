...
//		return;
		...
		AddButton( scene, "test: Action", ref pos, () => { var next_scene = MakeTestActionsScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: Pivot", ref pos, () => { var next_scene = MakeTestPivotScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: SpriteList", ref pos, () => { var next_scene = MakeManySpritesInSpriteListScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: Sprites ", ref pos, () => { var next_scene = MakeManySpritesScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: Plane3D", ref pos, () => { var next_scene = MakeTestPlane3D(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: Game scene", ref pos, () => { var next_scene = MakeTestGameScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: Forest scene", ref pos, () => { var next_scene = MakeForestScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: Isometric scene", ref pos, () => { var next_scene = MakeIsometricScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: Particles", ref pos, () => { var next_scene = MakeParticlesScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		AddButton( scene, "test: Multitouch", ref pos, () => { var next_scene = MakeMultiTouchScene(); AddSharedMenu( next_scene ); ReplaceScene( next_scene ); });
		
		{
			var b = AddButton( scene, get_transition_mode_string(), ref pos, () => { transition_mode=(transition_mode+1)%4; });
			b.Action += () => { b.Text = get_transition_mode_string(); };
		}
	}
