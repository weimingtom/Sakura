	...
	: IScene
{
    ...
    public void Setup( GraphicsContext graphics, Model model )
    {
		...
		shaderToon = new ShaderProgram( "/Application/Sample/Graphics/ShaderCatalogSample/shaders/Toon.cgx" );
		shaderToon.SetAttributeBinding( 0, "a_Position" );
		shaderToon.SetAttributeBinding( 1, "a_Normal" );

		texToon = new Texture2D( "/Application/Sample/Graphics/ShaderCatalogSample/data/toon.png", false ) ;
		texToon.SetFilter( Sce.Pss.Core.Graphics.TextureFilterMode.Nearest,
                           Sce.Pss.Core.Graphics.TextureFilterMode.Nearest,
                           Sce.Pss.Core.Graphics.TextureFilterMode.Disabled );
